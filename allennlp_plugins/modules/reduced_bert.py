...
@TokenEmbedder.register("reduced_pretrained_transformer")
class ReducedPretrainedTransformerEmbedder(TokenEmbedder):
    """
    ...
    def __init__(self, model_name: str,
        ...
        super().__init__()
        assert 'bert' in model_name, "暂不支持其他模型的某几层操作"
        ...
        assert 1 <= reduced_num <= 12
        self.transformer_model.config.num_hidden_layers = reduced_num
        self.transformer_model.encoder.layer = torch.nn.ModuleList(list(self.transformer_model.encoder.layer)[:reduced_num])
        ...
        class Lambda(nn.Module):
            ...
            def forward(self, x):
                # x = self.layernorm(x)
                return self.f(x)
        self.transformer_model.pooler = Lambda(lambda x: x)
